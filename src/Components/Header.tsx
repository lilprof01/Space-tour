    ...
    <header className="header fixed left-0 top-0 w-full flex items-center justify-between align-middle px-6 py-8 sm:p-0 z-[100]">
      ...
      <nav className="nav-container px-8 sm:flex justify-end items-center align-middle bg-transparent w-[85vw] lg:w-full hidden">
        <ul className="flex justify-between items-center align-middle gap-8 text-white text-[16px]">
          <NavLink to="/" end id="home" className="nav-links py-8">
            <span className="font-bold">00</span> HOME
          </NavLink>
          <NavLink
            to="/destination"
            id="destination"
            className="nav-links py-8"
          >
            <span className="font-bold">01</span> DESTINATION
          </NavLink>
          <NavLink to="crew" id="crew" className="nav-links py-8">
            <span className="font-bold">02</span> CREW
          </NavLink>
          <NavLink to="technologies" id="technology" className="nav-links py-8">
      ...
      >
        <div
          className={`h-1 w-12 bg-blue-300 ${openNav ? "burger1" : "burger-1"}`}
        ></div>
        <div
          className={`h-1 w-8 bg-blue-300 ${openNav ? "invisible" : "show"}`}
        ></div>
        <div
          className={`h-1 w-6 bg-blue-300 ${openNav ? "burger2" : "burger-2"}`}
        ></div>
      </div>
    </header>
  );
